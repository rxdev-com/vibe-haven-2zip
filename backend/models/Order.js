...
const orderSchema = new mongoose.Schema(
  {
    ...
    deliveryPreference: { type: String, default: "standard_same_day" },
    ...
    cancelledBy: { type: String, enum: ["vendor", "supplier", null], default: null },
...
orderSchema.pre("save", function (next) {
  if (!this.orderNumber) {
    this.orderNumber = `JB-${Date.now().toString(36).toUpperCase()}-${Math.floor(Math.random() * 1000).toString().padStart(3, "0")}`;
  }
  if (this.isNew) {
